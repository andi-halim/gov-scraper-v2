...
- **CDN bot-challenge bypass is a second Playwright path in `fetch_page()`.** `_is_bot_challenge(html, status, headers)` detects Cloudflare (cf-ray/Server header + 403, or body tokens), Akamai (AkamaiGHost + 403), and Azure WAF (body token) and retries with Playwright. The Playwright result is only accepted if the challenge signals are absent from the rendered HTML — if the challenge persists, the original status is kept. Header-based signals are gated to 403 to avoid retrying legitimate CDN-proxied 200 responses. This is consistent with `robots.txt` compliance: the bypass fires only after `robots.txt` allows crawling; CDN blocks are heuristic layers, not the site's stated policy.
- **`crawl_url()` accepts a `prefetched_seed` tuple** `(html, final_url, http_status, js_rendered)` to avoid re-fetching the seed URL. `run.py` always passes the result of its activity-check `fetch_page()` call as the prefetched seed. Without this, bot-bypassed URLs (where Playwright ran during the activity check) would re-trigger Playwright inside the depth crawler, and the second attempt often failed — causing the scorer to run on empty HTML and produce a score of 0.
